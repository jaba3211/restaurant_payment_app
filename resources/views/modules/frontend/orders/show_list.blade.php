@foreach($list as $row)
    <div class="card d-flex w-100 mb-3 shadow-sm flex-row" style="background: #76be622b;">
        ...
        <div class="card-body p-2">
            ...
            <p class="text-secondary fw-bold mb-2 text-end" style="font-size:20px">
                {{ $row->quantity }}X
            </p>
            <p class="text-warning fw-bold text-end mb-0" style="font-size:22px">
                {{ $row->quantity * $row->dish->price }}₾
            </p>
        </div>
    </div>
@endforeach
